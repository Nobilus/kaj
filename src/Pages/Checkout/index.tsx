import React, { DOMAttributes, FormEvent, FormEventHandler, useEffect, useState } from 'react';
      ...
      >
          ...
          {Object.keys(basket).map((key: any, index: any) => {
            ...
            return (
              <tr key={index}>
                ...
                <td style={border}>{basket[key].size ? basket[key].type : '/'}</td>
                <td style={border}>{basket[key].size ? basket[key].size : '/'}</td>
              </tr>
            );
          })}
        </tbody>
        <tfoot>
          ...
          <tr>
            ...
            <td></td>
            <td>€{total.toLocaleString('be-NL', { minimumFractionDigits: 2 })}</td>
          </tr>
        </tfoot>
      </table>
    );
    ...
    <>
      ...
      <div className="c-page">
        <form className="c-form" onSubmit={sendEmail}>
          <input className="c-form__input" type="text" name="voornaam" id="voornaam" placeholder="Voornaam" />
          <input className="c-form__input" type="text" name="naam" id="naam" placeholder="Naam" />
          <br className="br-tag" />
          ...
          <br className="br-tag" />
          <div>
            <input type="checkbox" name="afhaaling" id="afhaaling" onChange={() => (templateParams.afhaaling = !templateParams.afhaaling)} /> <label htmlFor="afhaaling">Afhalen?</label>
          </div>
          <br className="br-tag" />
          <ReCAPTCHA sitekey={siteKey} onChange={onChange} />
          <br className="br-tag" />
          <button className="o-button-reset c-form-button" type="submit">
            Bestellen
          </button>
        </form>
      </div>
